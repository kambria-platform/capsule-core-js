...
class WalletInterface {
  ...
  _watch = (callback) => {
    this.fetch().then(re => {
      ...
      // Network changed
      if (this.user.network !== util.getNetworkId(re.network, 'number')) {
  ...
  watch = (callback) => {
    ...
    // Called when new block coming
    this.filter.watch(() => {
      this._watch(callback);
    });
    // Web3ProviderEngine does not support synchronous
    return { stopWatching: () => { this.filter.stopWatching(() => { }) } };
  }
  ...
  static logout = () => {
    // Stop watching
    if (this.filter) this.filter.stopWatching(() => { });
    // Clear cache
    cache.removeAll();
    localStorage.remove();
