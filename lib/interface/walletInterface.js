...
class WalletInterface {
  ...
  getNetwork = () => {
    return new Promise((resolve, reject) => {
      ...
      return this.web3.eth.net.getId((er, re) => {
        console.log(0.1, er, re)
  ...
  getAccount = () => {
    return new Promise((resolve, reject) => {
      ...
      return this.web3.eth.getAccounts((er, accounts) => {
        console.log(0.2, er, accounts)
        if (er) return reject(er);
        if (accounts.length <= 0 || !accounts[0] || !this.isAddress(accounts[0]))
          return reject(ERROR.CANNOT_GET_ACCOUNT);
        return resolve(re[0]);
      });
    });
  }
  ...
  getBalance = (address) => {
    return new Promise((resolve, reject) => {
      ...
      return this.web3.eth.getBalance(address, 'latest', (er, re) => {
        console.log(0.3, er, re)
        if (er) return reject(er);
